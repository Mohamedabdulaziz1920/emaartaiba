...
export default function AddTestimonialClient({ breadcrumbs }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.agree_terms) {
      setFieldErrors({ agree_terms: ['يجب الموافقة على سياسة الخصوصية والشروط'] });
      return;
    }
    ...
    try {
      ...
      if (response && response.success === true) {
        setSuccess(true);
        // ✅ إظهار رسالة نجاح وإعادة التوجيه بعد 3 ثوان
        setTimeout(() => {
        ...
        if (response && response.errors) {
          ...
          setErrorMessage(response?.message || 'حدث خطأ في إرسال التقييم');
        }
      }
    } catch (err: any) {
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <input
                  ...
                  style={{
                    width: '100%',
                    padding: '0.75rem',
                    border: `1px solid ${fieldErrors.client_name ? '#dc2626' : '#e2e8f0'}`,
                    borderRadius: '0.5rem',
                    outline: 'none',
                    fontSize: '1rem'
                  }}
                ...
                {fieldErrors.client_name && (
                  <p style={{ color: '#dc2626', fontSize: '0.75rem', marginTop: '0.25rem' }}>{fieldErrors.client_name[0]}</p>
                )}
              </div>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <input
                  ...
                  style={{
                    width: '100%',
                    padding: '0.75rem',
                    border: `1px solid ${fieldErrors.client_email ? '#dc2626' : '#e2e8f0'}`,
                    borderRadius: '0.5rem',
                    outline: 'none',
                    fontSize: '1rem'
                  }}
                ...
                {fieldErrors.client_email && (
                  <p style={{ color: '#dc2626', fontSize: '0.75rem', marginTop: '0.25rem' }}>{fieldErrors.client_email[0]}</p>
                )}
              </div>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <textarea
                  ...
                  style={{
                    width: '100%',
                    padding: '0.75rem',
                    border: `1px solid ${fieldErrors.content ? '#dc2626' : '#e2e8f0'}`,
                    borderRadius: '0.5rem',
                    outline: 'none',
                    fontSize: '0.9375rem',
                    resize: 'vertical'
                ...
                />
                {fieldErrors.content && (
                  <p style={{ color: '#dc2626', fontSize: '0.75rem', marginTop: '0.25rem' }}>{fieldErrors.content[0]}</p>
                )}
              </div>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                {fieldErrors.agree_terms && (
                  <p style={{ color: '#dc2626', fontSize: '0.75rem', marginTop: '0.25rem' }}>{fieldErrors.agree_terms[0]}</p>
                )}
              </div>
